	...
	tests := []struct {
	...
	}{
		{
			...
			secret: createSecret("name", "namespace", ".dockerconfigjson",
				[]byte(`{ "auths": { "registry.ng.bluemix.net": { "username": "token", "password": "registry-token", "email": "email@email.com", "auth": "auth-token" } } }`)),
			secretName: "name",
			namespace:  "namespace",
		...
		},
		{
			...
			secret: createSecret("name", "namespace", ".dockercfg",
				[]byte(`{ "registry.ng.bluemix.net": { "username": "token", "password": "registry-token", "email": "email@email.com", "auth": "auth-token" } }`)),
			secretName: "name",
			namespace:  "namespace",
			registry:   "registry.ng.bluemix.net",
			wantUser:   "token",
